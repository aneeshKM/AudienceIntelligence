...
class _RunSupervisor:
    ...
    def start(self, request: StartRunRequest) -> dict[str, object]:
        with self._lock:
            existing = self._state(request.run_id)
    ...
    def _state(self, run_id: str) -> _RunState | None:
    ...
    def _persist_state(self, state: _RunState) -> None:
        path = self._state_path(state.run_id)
        path.parent.mkdir(parents=True, exist_ok=True)
        atomic_write_json(path, state.response())
